...
from models.diffusion_stock import DiffStock

class PM_Diffusion(PM):
    def __init__(self, args, data_all):
        self.identifier = f"diffusion_{id(self)}"  # Add identifier before calling super
        super(PM_Diffusion, self).__init__(args, data_all)
        self.logger.info(f"Initialized Diffusion model with identifier: {self.identifier}")
        

    def _build_model(self):
        """Build the Diffusion model with the specified parameters"""
        self.logger.info("Building Diffusion model")
        
        model = DiffStock(self.args)
        model = model.to(self.device)
        self.logger.info(f"Diffusion model structure:\n{model}")
        
        return model

    def _select_optimizer(self):
        """Select optimizer for Diffusion model"""
    ...
    def _select_criterion(self):
        """Select loss criterion for Diffusion model"""
        self.logger.info("Using MSE loss for Diffusion model")
        return nn.MSELoss()

    def train(self):
        """Train the Diffusion model on training data"""
        train_data, train_loader = self._get_data(flag="train")
        valid_data, valid_loader = self._get_data(flag="valid")
        
        self.logger.info(f"Starting Diffusion model training for {self.args.epochs} epochs")
        ...
        for epoch in range(self.args.epochs):
            ...
            if avg_valid_loss < best_valid_loss:
                ...
                # Save the best model
                best_model_path = os.path.join(checkpoint_dir, f"diffusion_best_{self.identifier}.pth")
                torch.save({
                    'epoch': epoch,
        ...
        # Save final model
        final_model_path = os.path.join(checkpoint_dir, f"diffusion_final_{self.identifier}.pth")
        torch.save({
            'model_state_dict': self.model.state_dict(),
        }, final_model_path)
        ...
        loss_plot_path = os.path.join(plots_dir, f"diffusion_loss_{self.identifier}.png")
    ...
    def test(self):
        """Test the Diffusion model on test data"""
        test_data, test_loader = self._get_data(flag="test")
        
        self.logger.info("Testing Diffusion model")
        ...
        plt.title('Diffusion: Predicted vs Actual')
        
        scatter_plot_path = os.path.join(plots_dir, f"diffusion_scatter_{self.identifier}.png")
    ...
    def backtest(self, topk: int = 5, holding_period: int = 5):
        ...
        for i in range(len(test_dataset.dataset)):
            ...
            if days_held == 0 or days_held >= holding_period:
                # Prepare input for Diffusion model (add batch dimension)
        ...
        plt.title(f'Diffusion Strategy (top-{topk}, hold-{holding_period}d) vs Market')
        plt.xlabel('Trading Days')
        plt.ylabel('Cumulative Return')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        returns_plot_path = os.path.join(plots_dir, f"diffusion_returns_{self.identifier}_top{topk}_hold{holding_period}.png")
        ...
        performance_file = os.path.join(log_dir, f'diffusion_performance_{self.identifier}_top{topk}_hold{holding_period}.csv')
        ...
        pickle_file = os.path.join(log_dir, f'diffusion_backtest_data_{self.identifier}_top{topk}_hold{holding_period}.pkl')
