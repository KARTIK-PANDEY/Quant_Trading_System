...
class PM_GRU(PM):
    def __init__(self, args, data_all):
        self.identifier = f"gru_{id(self)}"  # Add identifier before calling super
        super(PM_GRU, self).__init__(args, data_all)
        self.logger.info(f"Initialized GRU model with identifier: {self.identifier}")
    ...
    def _build_model(self):
        """Build the GRU model with the specified parameters"""
        self.logger.info("Building GRU model")
        
        model = GRU(self.args)
        model = model.to(self.device)
        self.logger.info(f"GRU model structure:\n{model}")
        
        return model

    def _select_optimizer(self):
        """Select optimizer for GRU model"""
    ...
    def _select_criterion(self):
        """Select loss criterion for GRU model"""
        self.logger.info("Using MSE loss for GRU model")
        return nn.MSELoss()

    def train(self):
        """Train the GRU model on training data"""
        train_data, train_loader = self._get_data(flag="train")
        valid_data, valid_loader = self._get_data(flag="valid")
        
        self.logger.info(f"Starting GRU model training for {self.args.epochs} epochs")
        ...
        for epoch in range(self.args.epochs):
            ...
            if avg_valid_loss < best_valid_loss:
                ...
                # Save the best model
                best_model_path = os.path.join(checkpoint_dir, f"gru_best_{self.identifier}.pth")
                torch.save({
                    'epoch': epoch,
        ...
        # Save final model
        final_model_path = os.path.join(checkpoint_dir, f"gru_final_{self.identifier}.pth")
        torch.save({
            'model_state_dict': self.model.state_dict(),
        }, final_model_path)
        ...
        loss_plot_path = os.path.join(plots_dir, f"gru_loss_{self.identifier}.png")
    ...
    def test(self):
        """Test the GRU model on test data"""
        test_data, test_loader = self._get_data(flag="test")
        
        self.logger.info("Testing GRU model")
        ...
        scatter_plot_path = os.path.join(plots_dir, f"gru_scatter_{self.identifier}.png")
    ...
    def backtest(self, topk: int = 5, holding_period: int = 5):
        ...
        for i in range(len(test_dataset.dataset)):
            ...
            if days_held == 0 or days_held >= holding_period:
                # Prepare input for GRU model (add batch dimension)
        ...
        plt.title(f'GRU Strategy (top-{topk}, hold-{holding_period}d) vs Market')
        plt.xlabel('Trading Days')
        plt.ylabel('Cumulative Return')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        returns_plot_path = os.path.join(plots_dir, f"gru_returns_{self.identifier}_top{topk}_hold{holding_period}.png")
        ...
        performance_file = os.path.join(log_dir, f'gru_performance_{self.identifier}_top{topk}_hold{holding_period}.csv')
        ...
        pickle_file = os.path.join(log_dir, f'gru_backtest_data_{self.identifier}_top{topk}_hold{holding_period}.pkl')
